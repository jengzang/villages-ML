...
**文件大小**: 2.3 GB (优化后，原 5.45 GB)
**总表数**: 44 张表 (优化后，原 45 张)

---

## 执行摘要

本项目数据库处于**优秀状态**，所有 15 个分析阶段已成功完成。数据库已完成优化，大小从 5.45 GB 减少到 2.3 GB（节省 58%），包含 44 张表，覆盖 285,860 个自然村的全面分析结果。
...
- **数据库大小**: 2.3 GB（优化后）
- **总行数**: 2.26M（优化后，原 21.16M）
- **索引数量**: 73（优化后，原 56）

### 数据库优化 (2026-02-24)

- ✅ 移除 run_id 冗余（只保留活跃版本）
- ✅ 合并频率/倾向性表为单一分析表
- ✅ 新增 17 个查询索引
- ✅ 空间节省: 3.15 GB (58%)
- ✅ 行数减少: 18.9M (89.3%)

**详细说明**: 参见 `docs/guides/DATABASE_OPTIMIZATION_COMPLETE.md`
...
**状态**: 完成
**表**:
- `char_frequency_global`: 3,844 行（全局字符频率，无 run_id）
- `char_regional_analysis`: 319,135 行（区域分析，合并频率+倾向性）
- `char_embeddings`: 9,209 行（字符向量，BLOB 格式）

**说明**: 已完成数据库优化，合并 `char_frequency_regional` 和 `regional_tendency` 为 `char_regional_analysis`。
...
- `semantic_vtf_global`: 9 行（全局虚拟词频，无 run_id）
- `semantic_regional_analysis`: 14,292 行（区域分析，合并频率+倾向性）
- `semantic_cooccurrence`: 数据已填充
- `semantic_network_edges`: 数据已填充

**说明**: 已完成数据库优化，合并 `semantic_vtf_regional` 和 `semantic_tendency` 为 `semantic_regional_analysis`。
