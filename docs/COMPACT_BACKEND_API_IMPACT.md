...
- Small metadata/basic-stat tables generated during preprocessing: `metadata_overview_stats`, `region_hierarchy_stats`, `regional_basic_stats`.
...
- `regional_basic_stats`: `region_key`, `region_level`, `region_name`, `city`, `county`, `township`, `village_count`, `avg_name_length`, `generated_at`, `data_version`.
...
| --- | --- | --- |
| `GET /regional/aggregates/city` | `T.VILLAGES_RAW` + `semantic_indices` | Must migrate off raw. Use `regional_basic_stats` for `village_count`/`avg_name_length` and `semantic_indices` for semantic percentages/counts. Keep supported after migration; disable in compact until migrated. |
| `GET /regional/aggregates/county` | `T.VILLAGES_RAW` + `semantic_indices` | Same as city. Must not scan raw in compact. |
| `GET /regional/aggregates/town` | `T.VILLAGES_RAW` + `semantic_indices` | Same as city. Treat API `town` as compact table `region_level='township'`. |
...
- `regional_basic_stats.village_count` for region counts.
- `regional_basic_stats.avg_name_length` for normalized-name average length.
- `semantic_indices.raw_intensity` and `semantic_indices.village_count` for semantic category percentages/counts.

`regional_basic_stats` is generated by the data pipeline in both full and compact profiles. Backend should use it as the unified source rather than branching full/compact query logic.
...
| --- | --- | --- |
| `GET /regional/aggregates/city` | `T.VILLAGES_RAW` | Disable in compact until migrated to `regional_basic_stats` + `semantic_indices` |
| `GET /regional/aggregates/county` | `T.VILLAGES_RAW` | Disable in compact until migrated to `regional_basic_stats` + `semantic_indices` |
| `GET /regional/aggregates/town` | `T.VILLAGES_RAW` | Disable in compact until migrated to `regional_basic_stats` + `semantic_indices` |
...
5. Migrate or disable raw-table realtime regional APIs.
   ...
   These APIs must not query raw in compact mode. Migrate `/regional/aggregates/*` to `regional_basic_stats` + `semantic_indices`. Migrate `/regional/vectors` hierarchy validation/reconstruction to `region_hierarchy_stats`.
