...
- **`.Codex/`**: Codex configuration and custom skills
...
This repository includes custom skills in the `.Codex/skills/` directory. Skills are reusable commands that can be invoked with `/skill-name` in Codex.

To use a skill, type `/skill-name` in the Codex CLI. Skills can accept parameters and provide specialized functionality for this project.
